    ...
    @slot('tbody')
        @foreach ($dbRequestsOpen as $dbRequestOpen)
            <x-table.tr>
                ...
                <x-table.td>                    
                    <x-button.minButtonEdit route="{{route('warehouse.centers.edit',['center'=>$dbDepartment->id,'request'=>$dbRequestOpen->id])}}" />
                    ...
                    <x-button.minButtonModalInfo id="Modal_{{$dbRequestOpen->id}}" title="Solicitação Nº {{$dbRequestOpen->code}}">
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            ...
                            <p class="col-span-2"><strong>Título:</strong> {{$dbRequestOpen->title ?? ""}}</p>
                            <p class="col-span-2"><strong>Estabelecimento:</strong> {{$dbRequestOpen->CompanyEstablishmentDepartment->CompanyEstablishment->title}}</p>
                            <p class="col-span-2"><strong>Departamento:</strong> {{$dbRequestOpen->CompanyEstablishmentDepartment->title}}</p>
                            <p><strong>Contato:</strong> {{$dbRequestOpen->department_contact ?? ""}}</p>
                            <p><strong>Ramal:</strong> {{$dbRequestOpen->department_extension ?? ""}}</p>
                            <hr class="col-span-2">
                            <p class="col-span-2"><strong>Usuário Solicitante:</strong> {{$dbRequestOpen->User->name}}</p>
