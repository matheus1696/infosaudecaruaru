<x-button.minButtonModalEdit id="request_details_{{$dbRequestDetail->id}}" title="Alterar quantidade da solicitação do suprimento">
    <x-form.form method="edit" route="{{route('foodstuff.centers.update',['center'=>$db->department_id,'request'=>$db->id])}}" title="Alterar Quantidade" color="sky">
        ...
        <x-form.input col="3" type="number" label="Quantidade" id="quantity" name="quantity" min="1" value="{{$dbRequestDetail->quantity_forwarded}}"/>
