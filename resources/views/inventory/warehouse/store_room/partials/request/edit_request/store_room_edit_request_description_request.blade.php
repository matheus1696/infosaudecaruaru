<div class="grid items-center justify-end grid-cols-12 col-span-12 gap-4 my-3">
    ...
    <p class="flex items-center col-span-12 text-sm md:col-span-12">
        ...
        @if ($db->status == "Encaminhado")
            {{$db->title}}
        @else
            <input type="text" id="title" name="title" class="flex-1 px-2 py-1 ml-1 border border-green-100 rounded-lg outline-none {{$db->title == NULL ? 'bg-red-100' : ''}}" value="{{$db->title ?? old('title')}}">
        @endif
    </p>
    ...
    <p class="flex items-center col-span-12 text-sm md:col-span-4">
        ...
        @else                  
            <input type="tel" id="department_contact" name="department_contact" class="px-2 py-1 ml-1 border border-green-100 rounded-lg outline-none {{$db->title == NULL ? 'bg-red-100' : ''}}" value="{{$db->department_contact ?? old('department_contact')}}" onkeyup="handlePhone(event)" maxlength="14" placeholder="(81) 0000-0000">
        @endif 
    </p>
    <p class="flex items-center col-span-12 text-sm md:col-span-4">
        <strong>Ramal: </strong>
        ...
        @else                  
            <input type="text" id="department_extension" name="department_extension" class="px-2 py-1 ml-1 border border-green-100 rounded-lg outline-none {{$db->title == NULL ? 'bg-red-100' : ''}}" value="{{$db->department_extension ?? old('department_extension')}}" placeholder="0000" maxlength="4">
        @endif 
    </p>
</div>