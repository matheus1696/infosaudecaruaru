...
<body
    class="bg-gradient-to-r from-green-400 to-green-600 min-h-screen flex flex-col items-center justify-start px-4 py-6 mt-10">
    <!-- Logo -->
    ...
    <div class="grid grid-cols-1 gap-6 w-full max-w-4xl">
        ...
        @if ($dbProfessionais->isEmpty())
            <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-lg shadow-md">
                ...
                <p>Você não está apto para fazer plantão extra no momento.</p>
            </div>
        @endif
            ...
            <div class="bg-white py-6 px-6 rounded-2xl shadow-lg transition-transform transform">
                <div class="flex items-center justify-between">
                    <div class="flex-1">
                        ...
                        <p class="text-green-500 text-sm mb-1">Matrícula: <span
                                class="font-semibold">{{ $dbProfessional->matricula }}</span></p>
                        <p class="text-green-500 text-sm mb-1">CPF: <span
                                class="font-semibold">{{ $dbProfessional->cpf }}</span></p>
                        <p class="text-green-500 text-sm mb-1">Lotação: <span
                                class="font-semibold">{{ $dbProfessional->lotacao }}</span></p>
                        <p class="text-green-500 text-sm mb-1">Cargo: <span
                                class="font-semibold">{{ $dbProfessional->cargo }}</span></p>
                        <p class="text-green-500 text-sm">Apto para Extra?
                            <span
                                class="font-semibold {{ $dbProfessional->apto_extra ? 'text-green-600' : 'text-red-600' }}">
