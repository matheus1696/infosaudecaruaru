<div class="grid items-center justify-end grid-cols-12 col-span-12 gap-4 my-3">
    ...
    <p class="flex items-center col-span-12 text-sm md:col-span-4">
        ...
        @else                  
            <input type="tel" id="department_contact" name="department_contact" class="px-2 py-1 ml-1 border border-green-100 rounded-lg outline-none {{$db->department_contact == NULL ? 'bg-red-100' : ''}}" value="{{$db->department_contact ?? old('department_contact')}}" onkeyup="handlePhone(event)" maxlength="14" placeholder="(81) 0000-0000">
        @endif 
    </p>
    <p class="flex items-center col-span-12 text-sm md:col-span-4">
        <strong>Ramal: </strong>
